...
int main() {
  ...
  char ch = getchar();  // 버퍼에 입력받은 스페이스바를 문자를 입력받는 함수를
                        // 이용해 제거
  ...
  for (int i = 0; i < STUDENTS; i++) {
    if (*(scores+i) >= 90) {  // scores[i]의 값이 조건에 맞으면 안의 코드를 실행
      grade = 'A';
               ...
               80) {  // scores[i]의 값이 조건에 맞으면 안의 코드를 실행
      grade = 'B';
    } else if (*(scores + i) >=
               70) {  // scores[i]의 값이 조건에 맞으면 안의 코드를 실행
      grade = 'C';
    } else if (*(scores + i) >=
               60) {  // scores[i]의 값이 조건에 맞으면 안의 코드를 실행
      grade = 'D';
...
double averageScores(int* scores) {
  ...
  printf(
      "평균 값은 %f입니다\n",
      ((double)sum) /
          STUDENTS);  // sum값을 double형으로 변형하고 학생수로 나눠 평균을 구함
  return 1;
}
